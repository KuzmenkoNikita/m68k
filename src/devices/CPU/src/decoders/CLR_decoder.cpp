...
CLR_Decoder::CLR_Decoder(std::shared_ptr<DataExchange::MemoryInterface> bus) : bus_(std::move(bus))
{

}

std::expected<DecodeResult, DecodeError> CLR_Decoder::decode(uint16_t opcodeWord, uint32_t instructionStartAddr) const
{
    InstructionData::CLR_InstructionData instructionData{};

    const auto sizeValue = (opcodeWord & SIZE_MASK) >> 6U; //NOLINT

    switch(sizeValue) {
        case 0b00: instructionData.size = OperationSize::BYTE;
        case 0b01: instructionData.size = OperationSize::WORD;
    ...
    const auto convertedAddressingModeData = convertAddressingModeData<InstructionData::CLR_InstructionData::AddressingModeData>(addressingModeData->data);

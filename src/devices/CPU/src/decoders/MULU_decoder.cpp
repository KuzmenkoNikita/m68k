...
constexpr uint16_t MODE_MASK = 0b11'1000;
constexpr uint16_t DATA_REG_MASK = 0b1110'0000'0000;


} //namespace


MULU_Decoder::MULU_Decoder(std::shared_ptr<DataExchange::MemoryInterface> bus) : bus_(std::move(bus))
{

}

std::expected<DecodeResult, DecodeError> MULU_Decoder::decode(uint16_t opcodeWord, uint32_t instructionStartAddr) const
{
    InstructionData::MULU_InstructionData instructionData{};

    instructionData.destinationDataRegister = (opcodeWord & DATA_REG_MASK) >> 9U; //NOLINT
    ...
    const auto convertedAddressingModeData = convertAddressingModeData<InstructionData::MULU_InstructionData::AddressingModeData>(addressingModeData->data);

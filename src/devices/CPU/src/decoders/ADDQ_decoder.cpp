...
constexpr uint16_t DATA_MASK = 0x0E00U;

} //namespace

ADDQ_Decoder::ADDQ_Decoder(std::shared_ptr<DataExchange::MemoryInterface> bus) : bus_(std::move(bus))
{

}

std::expected<DecodeResult, DecodeError> ADDQ_Decoder::decode(uint16_t opcodeWord, uint32_t instructionStartAddr) const
{
    InstructionData::ADDQ_InstructionData instructionData{};
    ...
    const auto dataValue = (opcodeWord & MODE_MASK) >> 9U; //NOLINT
    instructionData.data = dataValue == 0 ? 8 : dataValue; //NOLINT
    ...
    const auto convertedAddressingModeData = convertAddressingModeData<InstructionData::ADDQ_InstructionData::AddressingModeData>(addressingModeData->data);
